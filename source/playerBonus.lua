...
function PlayerBonus:start()
    ...
    if gameDataLoad then
        ...
        table.each(shopItems, function (shopItem)
            ...
            if shopItem.name == "Bolter Cannon" then
                shopItem.unlock = UnlockCannon("Bolter", gfx.image.new("images/player/cannons/cannon_simpleCannon"), "", nil, table.findByParam(cannonsData, "id", "blaster"),"Starter Weapon : Bolter",100, 0, 4, 1.5)
            end
